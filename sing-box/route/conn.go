...
func (m *ConnectionManager) connectionCopy(ctx context.Context, source net.Conn, destination net.Conn, direction bool, done *atomic.Bool, onClose N.CloseHandlerFunc) {
	defer func() {
		if r := recover(); r != nil {
			if done.Swap(true) {
				onClose(E.New("recovered from panic in connectionCopy: ", r))
			}
			common.Close(source, destination)
			m.logger.ErrorContext(ctx, "recovered from panic in connectionCopy: ", r)
		}
	}()
...
func (m *ConnectionManager) packetConnectionCopy(ctx context.Context, source N.PacketReader, destination N.PacketWriter, direction bool, done *atomic.Bool, onClose N.CloseHandlerFunc) {
	defer func() {
		if r := recover(); r != nil {
			if !done.Swap(true) {
				onClose(E.New("recovered from panic in packetConnectionCopy: ", r))
			}
			common.Close(source, destination)
			m.logger.ErrorContext(ctx, "recovered from panic in packetConnectionCopy: ", r)
		}
	}()
