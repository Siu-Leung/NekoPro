...
object CrashHandler : Thread.UncaughtExceptionHandler {
    ...
    fun buildReportHeader(): String {
        ...
        try {
            val sensitiveKeys = setOf("password", "psk", "secret", "key", "token", "uuid", "cert", "private")
            report += "Settings: \n"
            for (pair in PublicDatabase.kvPairDao.all()) {
                report += "\n"
                val isSensitive = sensitiveKeys.any { pair.key.contains(it, ignoreCase = true) }
                report += pair.key + ": " + if (isSensitive) "[REDACTED]" else pair.toString()
            }
        } catch (e: Exception) {
